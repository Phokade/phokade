			...
			<div className="flex flex-col gap-2 justify-center items-center w-[270px] border border-black p-9 rounded-lg">
				...
				<ul className="text-gray-600">
					...
					{features.map((feature) => (
						<li key={feature.id} className="mt-2 flex gap-2">
							<BiSolidCircle size={5} className="my-auto" />
							{feature.name}
						</li>
					))}
				</ul>
				<button className="w-fit py-2 px-10 rounded-lg hover:bg-yellow-500 duration-200 mt-16 border border-black">Beli Paket</button>
			</div>
		</>
	);
}
